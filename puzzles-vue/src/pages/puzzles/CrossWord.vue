...
const assignNumberToBoard = (rows: number, cols: number) => {
  ...
  // 既存データのリセット
  hintsInfo.vertical = {}
  hintsInfo.horizontal = {}
  hintNums.vertical = []
  hintNums.horizontal = []

  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      if (board.value[r][c].isBlack) continue
      ...
      if (!verticalCheckGrid[r][c] && r < rows - 1 && !board.value[r + 1][c].isBlack) {
        ...
        for (let i = r; i < rows; i++) {
          if (board.value[i][c].isBlack) break
          count++
          verticalCheckGrid[i][c] = true
        }
        const hintNum = board.value[r][c].number!
      ...
      if (!horizontalCheckGrid[r][c] && c < cols - 1 && !board.value[r][c + 1].isBlack) {
        ...
        for (let i = c; i < cols; i++) {
          if (board.value[r][i].isBlack) break
          count++
...
const submitNewEntries = () => {
  ...
  // console.log('board after generate:', board.value)
  // console.log('hintNums after assign:', hintNums)
